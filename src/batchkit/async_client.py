...
from .jobs import AsyncBatchJob
...
class AsyncBatchClient:
    def __init__(self, sdk: Any, *, storage_root: str | Path | None = None) -> None:
        self.provider = AsyncOpenAIProvider(sdk)
    ...
    async def resume(self, path_or_name: str | Path) -> AsyncBatchJob:
        path = Path(path_or_name)
        if path.exists():
            manifest_file = path if path.name == "manifest.json" else manifest_path(path)
            return AsyncBatchJob(self, manifest_file)
        ...
        return AsyncBatchJob(self, matches[-1])

    async def _submit_request_rows(
        self,
        *,
        name: str,
        request_rows: list[dict[str, Any]],
        ...
        file_id = await self.provider.upload_batch_file(requests_file)
        remote_batch = await self.provider.create_batch(input_file_id=file_id, metadata=metadata)
